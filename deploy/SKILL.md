...
4. Keep the full build/startup output logs in the agent session/memory.
5. If deployment succeeds (health check HTTP 200 OK), skip to Phase 6.
6. If deployment fails (build crash, Nginx 502, timeout), ensure the error logs/details are captured and proceed to Phase 5.

---

### Phase 5: Self-Healing & Redeployment Loop
**Action:** Invoke `/deploy-heal` with the captured error logs/details directly as arguments.
1. Let the healing agent diagnose the failure cause (missing dependency, wrong port, DB connection error) by parsing the passed error logs/details.
